...
int main() {
    ...
    if (window == NULL) { std::cout << "Gagal membuat window" << std::endl; glfwTerminate(); return -1; }
    ...
    glGenVertexArrays(1, &flashVAO); glGenBuffers(1, &flashVBO);
    glBindVertexArray(flashVAO); glBindBuffer(GL_ARRAY_BUFFER, flashVBO);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        flashModel = glm::rotate(flashModel, glm::radians((float)(rand() % 360)), glm::vec3(0.0f, 0.0f, 1.0f));
        ...
        if (ImGui::Button("TEMBAK SENJATA (Klik & Tahan)", ImVec2(300, 40))) isFiring = true;
        if (ImGui::IsItemActive()) isFiring = true; else isFiring = false;

        ImGui::Separator();
        ImGui::TextColored(ImVec4(0.0f, 0.8f, 1.0f, 1.0f), "Mode Kamera Inspektor:");
        
        if (ImGui::Button("Kamera Bebas (Manual)", ImVec2(280, 25))) {
            currentCamState = STATE_FREE; showFloatingUI = false;
        }
        ...
        if (ImGui::Button("Fokus Laras (Muzzle)", ImVec2(280, 25))) {
            ...
            currentTypeText = ""; typeIndex = 0; typeTimer = 0.0f; floatingUIPos = ImVec2(50, 80); showFloatingUI = true;
        }
        ...
        if (ImGui::Button("Fokus Magazine", ImVec2(280, 25))) {
            ...
            currentTypeText = ""; typeIndex = 0; typeTimer = 0.0f; floatingUIPos = ImVec2(50, 550); showFloatingUI = true;
        }
        ...
        if (ImGui::Button("Fokus Grip (Gagang)", ImVec2(280, 25))) {
            ...
            currentTypeText = ""; typeIndex = 0; typeTimer = 0.0f; floatingUIPos = ImVec2(1200, 750); showFloatingUI = true;
        }
        ...
        if (ImGui::Button("Fokus Optik (Scope)", ImVec2(280, 25))) {
            ...
            currentTypeText = ""; typeIndex = 0; typeTimer = 0.0f; floatingUIPos = ImVec2(50, 100); showFloatingUI = true;
        }
        ...
        // --- SISTEM TYPEWRITER (MULTI-PASS OUTLINE RENEDERING) ---
        if (showFloatingUI) {
            if (typeIndex < fullTypeText.length()) {
                typeTimer += deltaTime;
                if (typeTimer >= typeSpeed) {
                    typeTimer = 0.0f; typeIndex++;
            ...
            ImGuiWindowFlags floatingFlags = ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_AlwaysAutoResize;
            ...
            ImGui::SetCursorPos(ImVec2(pos.x - offset, pos.y - offset)); ImGui::Text("%s", displayText.c_str()); 
            ImGui::SetCursorPos(ImVec2(pos.x + offset, pos.y - offset)); ImGui::Text("%s", displayText.c_str()); 
            ImGui::SetCursorPos(ImVec2(pos.x - offset, pos.y + offset)); ImGui::Text("%s", displayText.c_str()); 
            ImGui::SetCursorPos(ImVec2(pos.x + offset, pos.y + offset)); ImGui::Text("%s", displayText.c_str()); 
            ImGui::PopStyleColor(); 
...
unsigned int loadCubemap(std::vector<std::string> faces) {
    ...
    for (unsigned int i = 0; i < faces.size(); i++) {
        ...
        } else { std::cout << "Cubemap texture gagal di path: " << faces[i] << std::endl; stbi_image_free(data); }
    }
